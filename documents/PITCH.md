...
Fallbacks: if Wi-Fi dies, the demo page does the same story with the Deen Shield stand-in.
If Chromium acts up, play the backup video.
...
flowchart LR
  subgraph Page["Web page (MAIN world)"]
    S[Site scripts] -->|fetch / img.src / canvas / navigator…| D[detector.js<br/>wraps APIs, attributes caller,<br/>rewrites probes when protected]
    F[protect-flag.js<br/>per protected site] -.sets flag.-> D
  end
  D -->|postMessage batches| B[bridge.js<br/>ISOLATED world]
  B -->|runtime.sendMessage| SW[service worker<br/>per-tab state · badge · storage]
  SW --> I[inference.js<br/>pure: events → traits]
  I --> P[popup card<br/>what happened · could reveal ·<br/>learned · Protect]
  P -->|Protect| SW -->|registerContentScripts| F
```

Say: "Nothing leaves the browser. No network blocking. The only judgement lives in one pure
module, which is why it's 97% covered."

## 5. Follow-up questions — prepared answers
